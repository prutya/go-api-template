...
import (
	...
	"prutya/go-api-template/internal/handlers/task_check"
...
func NewRouter(config *config.Config, logger *zap.Logger, tasksClient *asynq.Client) *Router {
	mux := chi.NewRouter()
	...
	mux.Post("/task-check", task_check.NewHandler(tasksClient))
